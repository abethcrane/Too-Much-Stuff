...
class Book(Item):
    ...
    def __init__(self, db, identifier):
        super(Book, self).__init__(db,identifier)
        self.id_type="ISBN"
        self.data["Category"] = "Book"
        self.data["ISBN"] = self.id
        self.data["Title"], self.data["Author"], self.data["Image"] = self.get_book_data(self.id)
	...
	book_data = None
        try:
            ...
            book_data = json.load(urllib2.urlopen(url))["items"][0]["volumeInfo"]
